...
std::vector<std::string> GenericJoypad::get_nodes() const {
...
std::vector<Joypad::UdevEvent> GenericJoypad::get_udev_events() const {
  return gen_joystick_udev_events(_state);
}

std::vector<Joypad::UdevHwDbEntry> GenericJoypad::get_udev_hw_db_entries() const {
  return gen_joystick_udev_hw_db(_state);
}

// Same button/axis set as create_xbox_controller/create_ps_controller (this
// backend has no per-vendor quirks to model), just a different USB identity —
// matches libvirtualhid's generic HID gamepad profile (0x1209:0x0001).
static Result<libevdev_uinput_ptr> create_generic_controller(const DeviceDefinition &device) {
...
GenericJoypad::GenericJoypad() : _state(std::make_shared<GenericJoypadState>()) {}

GenericJoypad::~GenericJoypad() {
  if (_state) {
...
Result<GenericJoypad> GenericJoypad::create(const DeviceDefinition &device) {
  auto joy_el = create_generic_controller(device);
  if (!joy_el) {
    return Error(joy_el.getErrorMessage());
  }

  GenericJoypad joypad;
...
void GenericJoypad::set_pressed_buttons(unsigned int newly_pressed) {
...
void GenericJoypad::set_stick(Joypad::STICK_POSITION stick_type, short x, short y) {
  if (auto controller = this->_state->joy.get()) {
    if (stick_type == LS) {
...
void GenericJoypad::set_triggers(int16_t left, int16_t right) {
  if (auto controller = this->_state->joy.get()) {
...
void GenericJoypad::set_on_rumble(const std::function<void(int, int)> &callback) {
